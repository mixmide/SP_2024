...
// КЛАСС ОБЪЕКТОВ "БАНКОВСКИЙ СЧЁТ" - из ДЗ-1
class bank_account {
    public:
        bank_account()
...
// КЛАСС ОБЪЕКТОВ - ДВУНАПРАВЛЕННЫЙ СПИСОК ИЗ ОБЪЕКТОВ "БАНКОВСКИЙ СЧЁТ" - из ДЗ-1
class list {
    public:
        list()      
...
// КЛАСС ОБЪЕКТОВ - ОЧЕРЕДЬ
class queue: public list {
    int max_size;           // максимальный размер очереди
    public:
        // конструктор, считаем значение 5 максимальным для очереди
        queue(const int max_size_enter = 5) : list()
        {
            max_size = max_size_enter;
            cout << "queue_object created!\n";
        }

        // деконструктор - корректно уничтожаем все объекты
        ~queue()
        {
            cout << "Destructor of queue_object worked!\n";
        }

        // добавления элемента в конец очереди
        void back(const bank_account& data) {
        ...
        //чтение первого элемента из очереди без его удаления
        bank_account* front() {
            return this->front_list();
        }
        ...
        //проверка, что очередь целиком заполнена
        bool full() {
            return (this->size()) >= max_size;
        }

        //вывод информации об элементах очереди без ее изменения
        void print() {
            this->print_list();
            cout << endl;
        }
...
int main(void)
{
    // создаем объекты класса "БАНКОВСКИЙ СЧЁТ"
    bank_account first(1234, "one", 623, 100);
    first.master_get();
    bank_account second(first);
    second.edit(12345, "two", 135, 200);
    second.master_get();
    bank_account third;
    third.edit(123456, "three", 111, 300);
    third.master_get();
    cout << "\n\n\n";
    ...
    queue1.back(second);
    queue1.back(third);
    cout << "Чтение элемента очереди: " << queue1.front() << "\n";
    queue1.pop();
    queue1.empty();
    cout << "Размер очереди: " << queue1.size() << "\n";
    cout << "Проверка на заполненность очереди (0 - no_max, 1 - max): " << queue1.full() << "\n";
    cout << "\nПечать информации об элементах очереди:\n";
    queue1.print();
    return 0;
}